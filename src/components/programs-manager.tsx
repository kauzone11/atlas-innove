...
export function ProgramsManager({ organizationId, programs, canManage }: { organizationId: string; programs: FundingProgramDto[]; canManage: boolean }) {
  return <div className="space-y-6">{canManage ? <CreateProgramForm organizationId={organizationId} /> : null}<section className="panel"><div className="panel-header"><h2>Programas da organização</h2><p>Cada programa pode reunir várias coortes de apoio.</p></div>{programs.length ? <div className="divide-y divide-line">{programs.map((program) => <Link key={program.id} href={`/app/programs/${program.id}`} className="flex min-h-20 items-center justify-between gap-4 px-6 py-5 transition-colors hover:bg-surface-subtle"><span className="min-w-0"><span className="block truncate font-medium text-ink">{program.name}</span><span className="mt-1 block text-sm text-slate">{program.code ? `${program.code} · ` : ""}{program.cohortCount} {program.cohortCount === 1 ? "coorte" : "coortes"}</span></span><span className="status-badge status-brand shrink-0">{statusLabel(program.status)}</span></Link>)}</div> : <p className="px-6 py-10 text-sm text-slate">Nenhum programa cadastrado. Crie o primeiro ciclo de apoio desta organização.</p>}</section></div>;
}
