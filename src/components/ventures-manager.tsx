...
export function VenturesManager({ organizationId, ventures, canManage, cohortFilter }: { organizationId: string; ventures: VentureDto[]; canManage: boolean; cohortFilter?: { id: string; name: string; programName: string } }) {
  return <div className="space-y-6">{cohortFilter ? <div className="flex flex-col gap-3 rounded-xl border border-line bg-white px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold uppercase tracking-[0.12em] text-accent">Filtro ativo</p><p className="mt-1 text-sm text-ink">{cohortFilter.name} · {cohortFilter.programName}</p></div><Link href="/app/ventures" className="text-sm font-semibold text-accent hover:text-accent-dark">Limpar filtro</Link></div> : null}{canManage && !cohortFilter ? <CreateVentureForm organizationId={organizationId} /> : null}<section className="rounded-2xl border border-line bg-white shadow-panel"><div className="border-b border-line px-6 py-5"><h2 className="font-semibold text-ink">{cohortFilter ? `Empreendimentos de ${cohortFilter.name}` : "Empreendimentos da organização"}</h2><p className="mt-1 text-sm text-slate">A identidade do empreendimento permanece estável entre diferentes coortes e programas.</p></div>{ventures.length ? <div className="divide-y divide-line">{ventures.map((venture) => <Link key={venture.id} href={`/app/ventures/${venture.id}`} className="flex items-center justify-between gap-4 px-6 py-5 hover:bg-canvas"><span className="min-w-0"><span className="block truncate font-medium text-ink">{venture.name}</span><span className="mt-1 block text-sm text-slate">{kindLabel(venture.kind)} · {venture.enrollments.length} {venture.enrollments.length === 1 ? "coorte" : "coortes"}</span></span><span className="shrink-0 text-sm font-semibold text-accent">Ver detalhes</span></Link>)}</div> : <p className="px-6 py-10 text-sm text-slate">{cohortFilter ? "Nenhum empreendimento está inscrito nesta coorte." : "Nenhum empreendimento cadastrado. Cadastre uma entidade para acompanhar suas participações."}</p>}</section></div>;
}
