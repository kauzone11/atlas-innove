...
export function VenturesManager({ organizationId, ventures, canManage, cohortFilter }: { organizationId: string; ventures: VentureDto[]; canManage: boolean; cohortFilter?: { id: string; name: string; programName: string } }) {
  return <div className="space-y-5">{cohortFilter ? <div className="flex flex-col gap-3 border-b border-line pb-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-medium text-slate">Filtro ativo</p><p className="mt-1 text-sm text-ink">{cohortFilter.name} · {cohortFilter.programName}</p></div><Link href="/app/ventures" className="button-tertiary min-h-9 px-2 text-xs">Limpar filtro</Link></div> : null}{canManage && !cohortFilter ? <CreateVentureDialog organizationId={organizationId} /> : null}<section className="panel"><div className="panel-header"><h2>{cohortFilter ? `Empreendimentos de ${cohortFilter.name}` : "Empreendimentos da organização"}</h2></div>{ventures.length ? <div className="divide-y divide-line">{ventures.map((venture) => <Link key={venture.id} href={`/app/ventures/${venture.id}`} className="flex min-h-20 items-center justify-between gap-4 px-6 py-4 transition-colors hover:bg-surface-subtle"><span className="min-w-0"><span className="block truncate font-medium text-ink">{venture.name}</span><span className="mt-1 block text-sm text-slate">{kindLabel(venture.kind)} · {venture.enrollments.length} {venture.enrollments.length === 1 ? "coorte" : "coortes"}</span></span><span className="shrink-0 text-sm font-semibold text-accent-hover">Ver detalhes</span></Link>)}</div> : <p className="px-6 py-10 text-sm text-slate">{cohortFilter ? "Nenhum empreendimento está inscrito nesta coorte." : "Nenhum empreendimento cadastrado. Cadastre uma entidade para acompanhar suas participações."}</p>}</section></div>;
}

function CreateVentureDialog({ organizationId }: { organizationId: string }) {
  const [open, setOpen] = useState(false);
  return <><div className="flex justify-end"><button type="button" className="button-primary" onClick={() => setOpen(true)}><Plus size={17} aria-hidden="true" /> Novo empreendimento</button></div><Dialog open={open} onClose={() => setOpen(false)} title="Novo empreendimento" description="Cadastre uma identidade que poderá participar de diferentes coortes."><CreateVentureForm organizationId={organizationId} onSuccess={() => setOpen(false)} /></Dialog></>;
}

function CreateVentureForm({ organizationId, onSuccess }: { organizationId: string; onSuccess?: () => void }) {
