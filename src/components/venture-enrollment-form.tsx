...
export function VentureEnrollmentForm({ organizationId, venture, cohorts, canManage }: { organizationId: string; venture: VentureDto; cohorts: CohortDto[]; canManage: boolean }) {
  ...
  return <form onSubmit={submit} className="panel p-6"><h2 className="font-semibold text-ink">Registrar em uma coorte</h2><p className="mt-1 text-sm text-slate">A participação é registrada separadamente da identidade do empreendimento.</p>{availableCohorts.length ? <div className="mt-5 grid gap-4 md:grid-cols-[1fr_1fr_auto] md:items-end"><label className="block space-y-2 text-sm font-medium text-ink"><span>Coorte</span><select required value={cohortId} onChange={(event) => setCohortId(event.target.value)} className="field-control"><option value="">Selecione uma coorte</option>{availableCohorts.map((cohort) => <option key={cohort.id} value={cohort.id}>{cohort.name} · {cohort.fundingProgram.name}</option>)}</select></label><label className="block space-y-2 text-sm font-medium text-ink"><span>Referência nesta participação</span><input value={externalReference} onChange={(event) => setExternalReference(event.target.value)} className="field-control" /></label><button disabled={pending} className="button-primary">{pending ? "Salvando…" : "Registrar"}</button></div> : <p className="mt-5 text-sm text-slate">Não há coortes elegíveis para este empreendimento.</p>}{notice ? <p className="mt-4 text-sm text-success" role="status">{notice}</p> : null}{error ? <p className="mt-4 text-sm text-danger" role="alert">{error}</p> : null}</form>;
}
