...
export default async function ProgramDetailPage({ params }: PageProps) {
  ...
  return <div className="space-y-8"><PageHeader breadcrumbs={<Breadcrumbs items={[{ label: "Programas", href: "/app/programs" }, { label: program.name }]} />} title={program.name} description={`${program.code ? `Código ${program.code} · ` : ""}${statusLabel(program.status)}`} action={<StatusBadge label={statusLabel(program.status)} tone={statusTone(program.status)} />} /><Panel className="p-6"><div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-slate"><span>Identificador: <strong className="font-medium text-ink">{program.slug}</strong></span>{program.description ? <span className="basis-full max-w-3xl leading-6">{program.description}</span> : null}</div></Panel><ProgramEditForm organizationId={context.organization.id} program={program} canManage={canManage} />{canManage ? <CohortCreateForm organizationId={context.organization.id} programId={program.id} /> : null}<Panel><PanelHeader title="Coortes" description="Ciclos de entrada para o acompanhamento dos empreendimentos." />{program.cohorts.length ? <div className="divide-y divide-line">{program.cohorts.map((cohort) => <Link key={cohort.id} href={`/app/programs/${program.id}/cohorts/${cohort.id}`} className="block px-6 py-5 transition-colors hover:bg-surface-subtle"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><h3 className="font-medium text-ink">{cohort.name}</h3><p className="mt-1 text-sm text-slate">{cohort.referenceYear ? `${cohort.referenceYear} · ` : ""}{dateRange(cohort.startsAt, cohort.endsAt)} · {cohort.ventureCount} {cohort.ventureCount === 1 ? "empreendimento inscrito" : "empreendimentos inscritos"}</p></div><span className="inline-flex shrink-0 items-center gap-2"><StatusBadge label={statusLabel(cohort.status)} tone={statusTone(cohort.status)} /><ArrowRight size={15} className="text-brand" aria-hidden="true" /></span></div></Link>)}</div> : <p className="px-6 py-10 text-sm text-slate">Nenhuma coorte cadastrada neste programa.</p>}</Panel></div>;
}
